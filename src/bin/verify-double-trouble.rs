...
// Because comm_r1 * 2 = comm_r2, we know that r = (s - s') / (challenge1 - 2*challenge2)
...
// a = s1 - challenge1 * r (or a = s2 - challenge2 * 2 * r)
...
// Use the public scalars u (computed & shared by prover in each proof) and the computed verifier challenges from proofs 1 and 2 to
// solve for ρ (the randomness used for comm_r in proof 1)
//  ρ = (u1 - u2) / (challenge1 - 2 * challenge2)
...
// alpha = u1 - challenge1 * rho1 (or alpha = u2 - challenge2 * rho2)
...
fn main() {
    ...
    // comm_r2 = 2 * comm_r <-- this is the key fact which will let us recover vector a and a's commitment randomness alpha
    assert_eq!(proof1.commitment.comm_r.mul(2), proof2.commitment.comm_r);

    // OVERVIEW
    // since comm_r2 = 2 * comm_r1, we can calculate the following 2 elements easily:
    // 1: r1 = (s1 - s2) / (challenge1 - 2 * challenge2)
    // 2: comm_r_rand = (u1 - u2) / (challenge1 - 2 * challenge2) (<-- this is r1's commitment randomness ρ, used to create comm_r)
    // then we will have everything needed to calculate our secret vector a and alpha (comm_a_rand), the randomness used to commit a

    // Get the verifier's random challenge values for proof1 and proof2
    // These will be used for recovering "random" vector r and random element ρ (comm_r_rand) used at the beginning of the online phase to create comm_r
    ...
    println!("secret vector a: {:?}", a);
    println!("secret randomness alpha: {:?}", comm_a_rand);
}
